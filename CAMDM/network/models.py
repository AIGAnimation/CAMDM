...
def seq_encoder_factory(arch: str,
                        latent_dim: int = 256,
                        ff_size: int = 1024,
                        num_layers: int = 8,
                        num_heads: int = 4,
                        ...
                        activation: str = "gelu"):
    """
    Constructs a sequence encoder model
    """
    if arch == 'trans_enc':
        ...
        return nn.TransformerEncoder(seqTransEncoderLayer, num_layers=num_layers)
    elif arch == 'trans_dec':
        ...
        seqTransDecoderLayer = nn.TransformerDecoderLayer(d_model=latent_dim,
                                                          ...
                                                          dropout=dropout,
                                                          activation=activation)
        return nn.TransformerDecoder(seqTransDecoderLayer, num_layers=num_layers)

    elif arch == 'gru':
        print("GRU init")
        return nn.GRU(latent_dim, latent_dim, num_layers=num_layers, batch_first=True)
    else:
        raise ValueError('Please choose correct architecture [trans_enc, trans_dec, gru]')


class MotionDiffusion(nn.Module):
    def __init__(self, input_feats, nstyles, njoints, nfeats,
        ...
        self.traj_trans_process = TrajProcess(2, latent_dim)
        self.traj_pose_process = TrajProcess(6, latent_dim)
        self.sequence_pos_encoder = PositionalEncoding(latent_dim, dropout)

        # global conditions
        self.embed_style = EmbedStyle(nstyles, latent_dim)
        ...
        self.output_process = OutputProcess(input_feats, latent_dim, njoints, nfeats)
