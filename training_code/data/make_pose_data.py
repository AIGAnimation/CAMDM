...
def process_motion(motion, use_scale=True):
    ...
    traj_trans, traj_angles, traj_poses = extract_traj(motion.global_positions[:, 0], forwards, smooth_kernel=[5, 10, 15, 20])
    return motion, {
...
if __name__ == '__main__':
    ...
    for bvh_path in tqdm(bvh_files):
        ...
        style_name = bvh_path.split('/')[-1].replace('.bvh', '').split('_')[0]
        action_label = bvh_path.split('/')[-1].replace('.bvh', '').split('_')[1]
