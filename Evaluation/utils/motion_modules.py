...
def on_ground(motion: Motion, joint_idx=None):
    global_pos = motion.global_positions
    if joint_idx is None:
        lowest_height = np.min(global_pos[:, :, 1])
    else:
        lowest_height = np.min(global_pos[:, joint_idx, 1])
...
def cal_metric_foot_sliding(motion: Motion, y_threshold=0.01):
    foot_idx = []
    ...
    foot_positions = motion.global_positions[:, foot_idx]
    contact = foot_positions[..., 1] < y_threshold 
    xz_positions = foot_positions[..., [0, 2]] 
    xz_differences = np.diff(xz_positions, axis=0)
    xz_differences = np.concatenate([xz_differences, np.zeros((1, xz_differences.shape[1], xz_differences.shape[2]))], axis=0)
    xz_distances = np.sqrt((xz_differences ** 2).sum(axis=-1)) 
    sliding_distances = xz_distances[contact]  # shape: (N,)

    root_distance = np.mean(np.linalg.norm(np.diff(motion.global_positions[:, 0, [0, 2]], axis=0), axis=-1))
    root_distance = 1 if root_distance == 0 else root_distance

    mean_sliding_distance = sliding_distances.mean() / root_distance
    return 0 if np.isnan(mean_sliding_distance) else mean_sliding_distance


def cal_metric_cohenrence(motion: Motion):
    differences = np.diff(motion.global_positions, axis=0)
    root_distance = np.mean(np.linalg.norm(np.diff(motion.global_positions[:, 0, [0, 2]], axis=0), axis=-1))
    root_distance = 1 if root_distance == 0 else root_distance
    distances = np.linalg.norm(differences, axis=-1) 
    return np.mean(np.mean(distances, axis=-1))/root_distance
